...
root.className = localStorage.getItem("currentTheme")
...
function setTheme() {
  const newTheme =
    root.className === "light-theme" ? "dark-theme" : "light-theme";
...
function setBookCardContent(bookInfo, bookIndex) {
  ...
  newBookCardElements[3].textContent =
    bookInfo.pages == 1 ? bookInfo.pages + " page" : bookInfo.pages + " pages";
...
function deleteBook() {
  ...
  if (
    confirm(
      `Are you sure you want to delete the book: ${myLibrary[bookIndex].title}?`
    )
  ) {
...
class Book {
  ...
  info() {
    return `${this.title} by ${this.author}, ${this.pages} pages long. ${
      this.beenRead ? "Has been read" : "Not read yet"
    } `;
  }
}
